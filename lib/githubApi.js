...
function triggerGithubPagesBuild(oOptions, sRepoPathBase) {
    ...
    return Request.post({
        ...
        headers: {
            "Authorization": `token ${oOptions.apiBasicAuth}`,
...
function newGithubFile (oOptions, sContent, sGithubPath, sSha) {
    ...
    return getGithubFile(oOptions, sGithubPath).then(
        (oResponse) => {
            console.log(`File at ${sGithubPath} was found with sha '${oResponse.sha}'`);
            if (!oResponse.sha) {
...
function _prepareGithubRequestData (sGithubPath, sPostData, apiBasicAuth) {
    const oRequestData = {
        ...
        headers: {
            ...
            "Authorization": `token ${apiBasicAuth}`,
            "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/49.0.2623.87 Safari/537.36"
        }
    };
    return oRequestData;
...
function _sendGithubRequest (oRequestData, sRequestBody) {
    return new Promise((fnResolve, fnReject) => {
        console.log("Sending request: " + JSON.stringify(oRequestData, null, 3));
